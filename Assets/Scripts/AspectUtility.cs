/*
 * Credit for majority of this class: http://wiki.unity3d.com/index.php?title=AspectRatioEnforcer
 */
...
public class AspectUtility : MonoBehaviour {
    ...
    private ScreenCollider screenCollider;
    ...
    void Update() {
        // check if there were ANY changes to the screen size/dimensions
        if (aspectRatio != Screen.width / Screen.height || currentScreenWidth != Screen.width || currentScreenHeight != Screen.height) {
            // adjust camera
            SetCamera();

            // adjust screen collider so ball doesn't bounce off screen
            screenCollider.UpdateEdgePoints();
    ...
    // Sets up camera for the desired aspect ratio w/ pillarbox/letterbox if necessary
